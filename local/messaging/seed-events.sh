...
aws --endpoint-url "$ENDPOINT_URL" sqs create-queue --queue-name bank-account-events >/dev/null
aws --endpoint-url "$ENDPOINT_URL" sqs send-message \
  --queue-url "$ENDPOINT_URL/000000000000/bank-account-events" \
  --message-body '{"event_id":"evt_123","account_id":"acc_123","action":"ACCOUNT_VIEWED"}' >/dev/null

echo "Seeded LocalStack SQS queue bank-account-events"
